# Given two strings, the first being a random string and the second being the same as the first, but with three added
...
# string2 = "aacccbbcc"
#
# // => 'c' You can assume that string2 will aways be larger than string1, and there will always be three added
...
def added_char(s1, s2):
    difference = ""
    for i in s2:
        if (i not in difference) and (s2.count(i) > s1.count(i)):
            difference += i
    return difference
